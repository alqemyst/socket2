...
impl Socket {
	...
	pub fn accept(&self) -> io::Result<(Socket, SockAddr)> {
		self.inner.accept().map(|(socket, addr)| (Socket { inner: socket }, addr))
	}
	...
	pub fn recv_from_with_flags(&self, buf: &mut [u8], flags: i32) -> io::Result<(usize, SockAddr)> {
	...
	/// Executes an operation of the `IP_ADD_SOURCE_MEMBERSHIP` type.
	#[cfg(target_os = "linux")]
	pub fn join_source_specific_multicast_v4(&self, multiaddr: &Ipv4Addr, sourceaddr: &Ipv4Addr, interface: &Ipv4Addr) -> io::Result<()> {
		self.inner.join_source_specific_multicast_v4(multiaddr, sourceaddr, interface)
	}
	...
	/// Executes an operation of the `IP_DROP_SOURCE_MEMBERSHIP` type.
	#[cfg(target_os = "linux")]
	pub fn leave_source_specific_multicast_v4(&self, multiaddr: &Ipv4Addr, sourceaddr: &Ipv4Addr, interface: &Ipv4Addr) -> io::Result<()> {
		self.inner.leave_source_specific_multicast_v4(multiaddr, sourceaddr, interface)
	}
...
mod test {
	...
	fn connect_timeout_unbound() {
		...
		match socket.connect_timeout(&addr, Duration::from_millis(250)) {
			Ok(_) => panic!("unexpected success"),
			Err(ref e) if e.kind() == io::ErrorKind::ConnectionRefused || e.kind() == io::ErrorKind::TimedOut => {}
			Err(e) => panic!("unexpected error {}", e),
		}
	}
