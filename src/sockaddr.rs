...
use libc::{in6_addr, in_addr, sa_family_t, sockaddr, sockaddr_in, sockaddr_in6, sockaddr_storage, socklen_t, AF_INET, AF_INET6};
#[cfg(windows)]
use winapi::shared::in6addr::{in6_addr_u, IN6_ADDR as in6_addr};
#[cfg(windows)]
use winapi::shared::inaddr::{in_addr_S_un, IN_ADDR as in_addr};
#[cfg(windows)]
use winapi::shared::ws2def::{ADDRESS_FAMILY as sa_family_t, AF_INET, AF_INET6, SOCKADDR as sockaddr, SOCKADDR_IN as sockaddr_in, SOCKADDR_STORAGE as sockaddr_storage};
#[cfg(windows)]
...
impl SockAddr {
	...
	pub unsafe fn from_raw_parts(addr: *const sockaddr, len: socklen_t) -> SockAddr {
		...
		ptr::copy_nonoverlapping(addr as *const _ as *const u8, storage.as_mut_ptr() as *mut u8, len as usize);
		...
		unsafe {
			...
			match (bytes.get(0), bytes.len().cmp(&addr.sun_path.len())) {
				...
				(Some(&0), Ordering::Greater) => {
					return Err(io::Error::new(io::ErrorKind::InvalidInput, "path must be no longer than SUN_LEN"));
				}
				(Some(&0), _) => {}
...
impl From<SocketAddrV4> for SockAddr {
	fn from(addr: SocketAddrV4) -> SockAddr {
		#[cfg(unix)]
		let sin_addr = in_addr { s_addr: u32::from_ne_bytes(addr.ip().octets()) };
		#[cfg(windows)]
		let sin_addr = unsafe {
		...
		let sockaddr_in = sockaddr_in {
			...
			#[cfg(any(target_os = "dragonfly", target_os = "freebsd", target_os = "haiku", target_os = "ios", target_os = "macos", target_os = "netbsd", target_os = "openbsd"))]
			sin_len: 0,
		};
		let mut storage = MaybeUninit::<sockaddr_storage>::zeroed();
...
impl From<SocketAddrV6> for SockAddr {
	fn from(addr: SocketAddrV6) -> SockAddr {
		#[cfg(unix)]
		let sin6_addr = in6_addr { s6_addr: addr.ip().octets() };
		#[cfg(windows)]
...
mod test {
	...
	#[test]
	fn inet6() {
		let raw = "[2001:db8::ff00:42:8329]:80".parse::<SocketAddrV6>().unwrap();
		let addr = SockAddr::from(raw);
