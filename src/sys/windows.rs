...
impl_debug!(crate::Type, ws2def::SOCK_STREAM, ws2def::SOCK_DGRAM, ws2def::SOCK_RAW, ws2def::SOCK_RDM, ws2def::SOCK_SEQPACKET,);

impl_debug!(crate::Protocol, self::IPPROTO_ICMP, self::IPPROTO_ICMPV6, self::IPPROTO_TCP, self::IPPROTO_UDP,);
...
impl Socket {
	pub fn new(family: c_int, ty: c_int, protocol: c_int) -> io::Result<Socket> {
		init();
		unsafe {
			let socket = match sock::WSASocketW(family, ty, protocol, ptr::null_mut(), 0, WSA_FLAG_OVERLAPPED) {
	...
	pub fn try_clone(&self) -> io::Result<Socket> {
		unsafe {
			...
			let socket = sock::WSASocketW(info.iAddressFamily, info.iSocketType, info.iProtocol, &mut info, 0, WSA_FLAG_OVERLAPPED);
	...
	pub fn recv(&self, buf: &mut [u8], flags: c_int) -> io::Result<usize> {
		unsafe {
			let n = { sock::recv(self.socket, buf.as_mut_ptr() as *mut c_char, clamp(buf.len()), flags) };
			match n {
	...
	pub fn peek(&self, buf: &mut [u8]) -> io::Result<usize> {
		unsafe {
			let n = { sock::recv(self.socket, buf.as_mut_ptr() as *mut c_char, clamp(buf.len()), MSG_PEEK) };
			match n {
				sock::SOCKET_ERROR if sock::WSAGetLastError() == sock::WSAESHUTDOWN as i32 => Ok(0),
				sock::SOCKET_ERROR => Err(last_error()),
				n => Ok(n as usize),
	...
	pub fn recv_from(&self, buf: &mut [u8], flags: c_int) -> io::Result<(usize, SockAddr)> {
		unsafe {
			...
			let n = { sock::recvfrom(self.socket, buf.as_mut_ptr() as *mut c_char, clamp(buf.len()), flags, &mut storage as *mut _ as *mut _, &mut addrlen) };
			let n = match n {
	...
	pub fn send(&self, buf: &[u8], flags: c_int) -> io::Result<usize> {
		unsafe {
			let n = { sock::send(self.socket, buf.as_ptr() as *const c_char, clamp(buf.len()), flags) };
	...
	pub fn send_to(&self, buf: &[u8], flags: c_int, addr: &SockAddr) -> io::Result<usize> {
		unsafe {
			let n = { sock::sendto(self.socket, buf.as_ptr() as *const c_char, clamp(buf.len()), flags, addr.as_ptr(), addr.len()) };
			if n == sock::SOCKET_ERROR {
				Err(last_error())
			} else {
				Ok(n as usize)
	...
	pub fn set_multicast_loop_v6(&self, multicast_loop_v6: bool) -> io::Result<()> {
		unsafe { self.setsockopt(IPPROTO_IPV6 as c_int, IPV6_MULTICAST_LOOP, multicast_loop_v6 as c_int) }
	}
	...
	pub fn keepalive(&self) -> io::Result<Option<Duration>> {
		...
		let n = unsafe { sock::WSAIoctl(self.socket, SIO_KEEPALIVE_VALS, 0 as *mut _, 0, &mut ka as *mut _ as *mut _, mem::size_of_val(&ka) as DWORD, 0 as *mut _, 0 as *mut _, None) };
		if n == 0 {
			Ok(if ka.onoff == 0 {
				None
	...
	pub fn set_keepalive(&self, keepalive: Option<Duration>) -> io::Result<()> {
		...
		let mut out = 0;
		let n = unsafe { sock::WSAIoctl(self.socket, SIO_KEEPALIVE_VALS, &ka as *const _ as *mut _, mem::size_of_val(&ka) as DWORD, 0 as *mut _, 0, &mut out, 0 as *mut _, None) };
		if n == 0 {
			Ok(())
		} else {
			Err(last_error())
	...
	unsafe fn getsockopt<T: Copy>(&self, opt: c_int, val: c_int) -> io::Result<T> {
		...
		if sock::getsockopt(self.socket, opt, val, &mut slot as *mut _ as *mut _, &mut len) == 0 {
...
impl FromRawSocket for crate::Socket {
	unsafe fn from_raw_socket(socket: RawSocket) -> crate::Socket {
		crate::Socket { inner: Socket::from_raw_socket(socket) }
	}
}
	...
	match dur {
		Some(dur) => {
			...
			let ms = dur
				...
				.map(|ms| if ms > <DWORD>::max_value() as u64 { INFINITE } else { ms as DWORD })
				.unwrap_or(INFINITE);
			if ms == 0 {
				return Err(io::Error::new(io::ErrorKind::InvalidInput, "cannot set a 0 duration timeout"));
			}
...
fn to_s_addr(addr: &Ipv4Addr) -> in_addr_S_un {
	...
	let res = crate::hton(((octets[0] as ULONG) << 24) | ((octets[1] as ULONG) << 16) | ((octets[2] as ULONG) << 8) | ((octets[3] as ULONG) << 0));
...
fn dur2linger(dur: Option<Duration>) -> sock::linger {
	match dur {
		Some(d) => sock::linger { l_onoff: 1, l_linger: d.as_secs() as u16 },
