...
impl_debug!(crate::Protocol, libc::IPPROTO_ICMP, libc::IPPROTO_ICMPV6, libc::IPPROTO_TCP, libc::IPPROTO_UDP,);
...
impl Socket {
	...
	pub fn connect_timeout(&self, addr: &SockAddr, timeout: Duration) -> io::Result<()> {
		...
		loop {
			...
			let mut timeout = timeout.as_secs().saturating_mul(1_000).saturating_add(timeout.subsec_nanos() as u64 / 1_000_000);
			if timeout == 0 {
				timeout = 1;
			}
			...
			match unsafe { libc::poll(&mut pollfd, 1, timeout) } {
				...
				_ => {
					...
					if pollfd.revents & libc::POLLHUP != 0 {
						let e = self.take_error()?.unwrap_or_else(|| io::Error::new(io::ErrorKind::Other, "no error set after POLLHUP"));
						return Err(e);
					}
					return Ok(());
				}
	...
	pub fn local_addr(&self) -> io::Result<SockAddr> {
		unsafe {
			...
			cvt(libc::getsockname(self.fd, &mut storage as *mut _ as *mut _, &mut len))?;
	...
	pub fn peer_addr(&self) -> io::Result<SockAddr> {
		unsafe {
			let mut storage: libc::sockaddr_storage = mem::zeroed();
			let mut len = mem::size_of_val(&storage) as libc::socklen_t;
			cvt(libc::getpeername(self.fd, &mut storage as *mut _ as *mut _, &mut len))?;
			Ok(SockAddr::from_raw_parts(&storage as *const _ as *const _, len))
		}
	}
	...
	pub fn accept(&self) -> io::Result<(Socket, SockAddr)> {
		...
		#[cfg(target_os = "linux")]
		{
			let res = cvt_r(|| unsafe { libc::syscall(libc::SYS_accept4, self.fd as libc::c_long, &mut storage as *mut _ as libc::c_long, &mut len, libc::SOCK_CLOEXEC as libc::c_long) as libc::c_int });
			match res {
		...
		let socket = match socket {
			Some(socket) => socket,
			None => unsafe {
				let fd = cvt_r(|| libc::accept(self.fd, &mut storage as *mut _ as *mut _, &mut len))?;
				let fd = Socket::from_raw_fd(fd);
				set_cloexec(fd.as_raw_fd())?;
				fd
			},
	...
	pub fn recv(&self, buf: &mut [u8], flags: c_int) -> io::Result<usize> {
		unsafe {
			let n = cvt(libc::recv(self.fd, buf.as_mut_ptr() as *mut c_void, cmp::min(buf.len(), max_len()), flags))?;
			Ok(n as usize)
		}
	}
	...
	pub fn peek(&self, buf: &mut [u8]) -> io::Result<usize> {
		unsafe {
			let n = cvt(libc::recv(self.fd, buf.as_mut_ptr() as *mut c_void, cmp::min(buf.len(), max_len()), libc::MSG_PEEK))?;
			Ok(n as usize)
		}
	}
	...
	pub fn recv_from(&self, buf: &mut [u8], flags: c_int) -> io::Result<(usize, SockAddr)> {
		unsafe {
			...
			let n = cvt(libc::recvfrom(self.fd, buf.as_mut_ptr() as *mut c_void, cmp::min(buf.len(), max_len()), flags, &mut storage as *mut _ as *mut _, &mut addrlen))?;
	...
	pub fn send(&self, buf: &[u8], flags: c_int) -> io::Result<usize> {
		unsafe {
			let n = cvt(libc::send(self.fd, buf.as_ptr() as *const c_void, cmp::min(buf.len(), max_len()), flags))?;
			Ok(n as usize)
		}
	}
	...
	pub fn send_to(&self, buf: &[u8], flags: c_int, addr: &SockAddr) -> io::Result<usize> {
		unsafe {
			let n = cvt(libc::sendto(self.fd, buf.as_ptr() as *const c_void, cmp::min(buf.len(), max_len()), flags, addr.as_ptr(), addr.len()))?;
			Ok(n as usize)
		}
	}
	...
	pub fn device(&self) -> io::Result<Option<CString>> {
		...
		let mut buf: [MaybeUninit<u8>; libc::IFNAMSIZ] = unsafe { MaybeUninit::<[MaybeUninit<u8>; libc::IFNAMSIZ]>::uninit().assume_init() };
		let mut len = buf.len() as libc::socklen_t;
		let len = unsafe { cvt(libc::getsockopt(self.fd, libc::SOL_SOCKET, libc::SO_BINDTODEVICE, buf.as_mut_ptr().cast(), &mut len))? };
		if len == 0 {
			Ok(None)
		} else {
	...
	pub fn bind_device(&self, interface: Option<&CStr>) -> io::Result<()> {
		let (value, len) = if let Some(interface) = interface { (interface.as_ptr(), interface.to_bytes_with_nul().len()) } else { (ptr::null(), 0) };

		unsafe { cvt(libc::setsockopt(self.fd, libc::SOL_SOCKET, libc::SO_BINDTODEVICE, value.cast(), len as libc::socklen_t)).map(|_| ()) }
	}
	...
	pub fn set_unicast_hops_v6(&self, hops: u32) -> io::Result<()> {
		unsafe { self.setsockopt(libc::IPPROTO_IPV6 as c_int, libc::IPV6_UNICAST_HOPS, hops as c_int) }
	}
	...
	pub fn read_timeout(&self) -> io::Result<Option<Duration>> {
		unsafe { Ok(timeval2dur(self.getsockopt(libc::SOL_SOCKET, libc::SO_RCVTIMEO)?)) }
	}
	...
	pub fn write_timeout(&self) -> io::Result<Option<Duration>> {
		unsafe { Ok(timeval2dur(self.getsockopt(libc::SOL_SOCKET, libc::SO_SNDTIMEO)?)) }
	}
	...
	pub fn set_multicast_loop_v4(&self, multicast_loop_v4: bool) -> io::Result<()> {
		unsafe { self.setsockopt(libc::IPPROTO_IP, libc::IP_MULTICAST_LOOP, multicast_loop_v4 as c_int) }
	}
	...
	pub fn set_multicast_ttl_v4(&self, multicast_ttl_v4: u32) -> io::Result<()> {
		unsafe { self.setsockopt(libc::IPPROTO_IP, libc::IP_MULTICAST_TTL, multicast_ttl_v4 as c_int) }
	}
	...
	pub fn set_multicast_loop_v6(&self, multicast_loop_v6: bool) -> io::Result<()> {
		unsafe { self.setsockopt(libc::IPPROTO_IPV6, libc::IPV6_MULTICAST_LOOP, multicast_loop_v6 as c_int) }
	}
	...
	#[cfg(target_os = "linux")]
	pub fn join_source_specific_multicast_v4(&self, multiaddr: &Ipv4Addr, sourceaddr: &Ipv4Addr, interface: &Ipv4Addr) -> io::Result<()> {
		let multiaddr = to_s_addr(multiaddr);
	...
	#[cfg(target_os = "linux")]
	pub fn leave_source_specific_multicast_v4(&self, multiaddr: &Ipv4Addr, sourceaddr: &Ipv4Addr, interface: &Ipv4Addr) -> io::Result<()> {
		let multiaddr = to_s_addr(multiaddr);
		let sourceaddr = to_s_addr(sourceaddr);
		let interface = to_s_addr(interface);
		let mreq_source = libc::ip_mreq_source {
	...
	pub fn linger(&self) -> io::Result<Option<Duration>> {
		unsafe { Ok(linger2dur(self.getsockopt(libc::SOL_SOCKET, libc::SO_LINGER)?)) }
	}
	...
	pub fn set_keepalive(&self, keepalive: Option<Duration>) -> io::Result<()> {
		unsafe {
			self.setsockopt(libc::SOL_SOCKET, libc::SO_KEEPALIVE, keepalive.is_some() as c_int)?;
		...
		T: Copy,
	{
		...
		cvt(libc::setsockopt(self.fd, opt, val, payload, mem::size_of::<T>() as libc::socklen_t))?;
		Ok(())
	}
	...
	unsafe fn getsockopt<T: Copy>(&self, opt: c_int, val: c_int) -> io::Result<T> {
		...
		cvt(libc::getsockopt(self.fd, opt, val, &mut slot as *mut _ as *mut _, &mut len))?;
...
impl<'a> Read for &'a Socket {
	fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
		unsafe {
			let n = cvt(libc::read(self.fd, buf.as_mut_ptr() as *mut c_void, cmp::min(buf.len(), max_len())))?;
			Ok(n as usize)
		}
	}
}
...
impl FromRawFd for crate::Socket {
	unsafe fn from_raw_fd(fd: c_int) -> crate::Socket {
		crate::Socket { inner: Socket::from_raw_fd(fd) }
	}
}
...
fn dur2timeval(dur: Option<Duration>) -> io::Result<libc::timeval> {
	match dur {
		Some(dur) => {
			...
			let secs = if dur.as_secs() > libc::time_t::max_value() as u64 { libc::time_t::max_value() } else { dur.as_secs() as libc::time_t };
...
fn to_s_addr(addr: &Ipv4Addr) -> libc::in_addr_t {
	...
	crate::hton(((octets[0] as libc::in_addr_t) << 24) | ((octets[1] as libc::in_addr_t) << 16) | ((octets[2] as libc::in_addr_t) << 8) | ((octets[3] as libc::in_addr_t) << 0))
}
